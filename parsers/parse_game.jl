...
function parse_game(json_file)
    open(json_file,"r") do f
        ...
        agents_names = Vector{String}([agent for agent in GameDict["agents"]])
        ...
        for loc in GameDict["locations"]
            ...
            invariant::Constraint = parse(loc["invariant"], Bindings(Vector([]), Vector([]), variables), constraint)
            flow::ReAssignment = OrderedDict{Symbol, ExprLike}()
            for reassinment in loc["flow"]
                flow[first(keys(reassinment))] = parse(first(values(reassinment)), Bindings(Vector([]), Vector([]), variables), expression)
            end
        ...
        for edge in GameDict["edges"]
            ...
            guard::Constraint = parse(edge["guard"], Bindings(Vector([]), Vector([]), variables), constraint)
            jump::ReAssignment = OrderedDict{Symbol, ExprLike}()
            for reassinment in edge["jump"]
                jump[first(keys(reassinment))] = parse(first(values(reassinment)), Bindings(Vector([]), Vector([]), variables), expression)
            end
        ...
        triggers::Dict{Agent, Vector{Constraint}} = Dict(Symbol(agent) => 
            Constraint[parse(trigger, Bindings(Vector([]), Vector([]), variables), constraint)
